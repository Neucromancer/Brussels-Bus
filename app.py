...
def build_route_map(
    ...
    start_id: str,
    goal_id: str,
    user_start_coords: Optional[Coords] = None, # Thêm tham số Tọa độ xuất phát
    user_goal_coords: Optional[Coords] = None   # Thêm tham số Tọa độ đích
) -> Optional[folium.Map]:
    rows = collect_route_rows(path_nodes, stop_lookup)
    coords = []
    route_points = []
    ...
    # Tạo danh sách toàn bộ tọa độ (bao gồm cả điểm đi bộ) để căn giữa bản đồ
    all_coords = list(coords)
    if user_start_coords:
        all_coords.insert(0, (user_start_coords.lat, user_start_coords.lon))
    if user_goal_coords:
        all_coords.append((user_goal_coords.lat, user_goal_coords.lon))
    ...
    # 1. Vẽ đường di chuyển bằng xe Bus (Nét liền màu xanh - Code cũ của bạn)
    if len(coords) >= 2:
        folium.PolyLine(
            coords,
            color="#1f77b4",
            weight=5,
            opacity=0.85,
            tooltip="Lộ trình tuyến xe",
        ).add_to(m)

    # 2. Vẽ đường đi bộ (Nét đứt màu xám)
    if user_start_coords and coords:
        folium.PolyLine(
            [(user_start_coords.lat, user_start_coords.lon), coords[0]],
            color="gray", weight=4, dash_array="5, 10", opacity=0.8, tooltip="Đi bộ ra bến"
        ).add_to(m)
        
    if user_goal_coords and coords:
        folium.PolyLine(
            [coords[-1], (user_goal_coords.lat, user_goal_coords.lon)],
            color="gray", weight=4, dash_array="5, 10", opacity=0.8, tooltip="Đi bộ đến đích"
        ).add_to(m)

    # 3. Thêm Marker cho các bến xe (Code cũ của bạn)
    for i, point in enumerate(route_points):
        ...
        if is_start:
            ...
            icon = "play"
            prefix = "fa"
            label = "Bến đi" # Đổi tên một chút để phân biệt với Vị trí thực tế
        elif is_goal:
            color = "red"
            icon = "flag"
            prefix = "fa"
    ...
    # 4. Thêm Marker cho Vị Trí Xuất Phát (Màu đen - icon User)
    if user_start_coords:
        folium.Marker(
            location=[user_start_coords.lat, user_start_coords.lon],
            popup="<b>Vị trí xuất phát của bạn</b>",
            tooltip="Vị trí xuất phát",
            icon=folium.Icon(color="red", icon="user", prefix="fa"),
        ).add_to(m)

    # 5. Thêm Marker cho Vị Trí Đích Đến (Màu đen - icon Star)
    if user_goal_coords:
        folium.Marker(
            location=[user_goal_coords.lat, user_goal_coords.lon],
            popup="<b>Điểm đến thực tế</b>",
            tooltip="Điểm đến thực tế",
            icon=folium.Icon(color="green", icon="star", prefix="fa"),
        ).add_to(m)

    # Căn chỉnh lại khung nhìn bản đồ để bao trọn cả điểm đi bộ
...
if run:
    ...
    if not results:
        ...
        # Truyền thêm start_coords và goal_coords vào hàm
        map_obj = build_route_map(
            path_nodes, 
            stop_lookup, 
            start_id=start_id, 
            goal_id=goal_id,
            user_start_coords=start_coords,
            user_goal_coords=goal_coords
        )
