...
def a_star_search(user_coords, dest_coords, all_stops):
    ...
    start_stops = find_nearest_stops(user_lat=user_coords.lat, user_lon=user_coords.lon, all_stops=all_stops, radius_km=1.2) # Tìm 3 bến gần nhất với điểm bắt đầu
    goal_stops = find_nearest_stops(user_lat=dest_coords.lat, user_lon=dest_coords.lon, all_stops=all_stops, radius_km=1.2) # Tìm 3 bến gần nhất với điểm đích
    ...
    for s_stop in start_stops:
        ...
        g_start = dist_to_minutes(d_walk, speed_kmh=WALKING_SPEED) # Đi bộ 3km/h
    ...
    while queue:
        ...
        # 2. KIỂM TRA NGƯỠNG TỐI ƯU ĐỂ TỈA NHÁNH (Pruning)

        # Nếu chi phí dự kiến đã vượt quá UPPER_BOUND_FACTOR lần đường tốt nhất tìm thấy -> Dừng duyệt
